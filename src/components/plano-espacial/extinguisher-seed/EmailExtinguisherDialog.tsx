...
───────────────────────────────────────────────────
  ...
  • Moneda              : ${currency}
───────────────────────────────────────────────────
  RESUMEN FINANCIERO Y DE CUMPLIMIENTO NORMAS (STPS/SSA3)
───────────────────────────────────────────────────
...
───────────────────────────────────────────────────
Este correo es generado por Low Voltage Estimator - Módulo Plano Espacial.
`;
}
...
export function EmailExtinguisherDialog({
  ...
  const handleSendOutlook = async () => {
    ...
    try {
      ...
      // 2. Armar el cuerpo y disparar mailto:
      const body = buildEmailBody(meta, itemsWithPrice);
      const ccParam = ccList.length > 0 ? `&cc=${encodeURIComponent(ccList.join(';'))}` : '';
      const mailtoUri = `mailto:${encodeURIComponent(to.trim())}?subject=${encodeURIComponent(
        subject
