...
import { NOM026SignCategory } from '@/types/emergencySignage';
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
import {
  EmergencyExitParametricReportDialog,
  EmergencyExitParametricItem,
} from './EmergencyExitParametricReportDialog';
...
export function EmergencyExitSummarySheet({
  ...
  const [showParametricReport, setShowParametricReport] = useState(false);

  const itemsWithPrice: EmergencyExitParametricItem[] = useMemo(() => {
    const categoryDetails: Record<
      NOM026SignCategory,
      { sku: string; name: string; unitCost: number; installCost: number }
    > = {
      SALIDA_DE_EMERGENCIA: {
        sku: 'NOM026-EMG-EXIT',
        name: 'Señal Salida de Emergencia Directa',
        unitCost: 185.0,
        installCost: 65.0,
      },
      RUTA_DE_EVACUACION: {
        sku: 'NOM026-EMG-ROUTE',
        name: 'Señal Ruta de Evacuación (Direccional)',
        unitCost: 140.0,
        installCost: 55.0,
      },
      ESCALERA_DE_EMERGENCIA: {
        sku: 'NOM026-EMG-STAIR',
        name: 'Señal Escalera de Emergencia',
        unitCost: 165.0,
        installCost: 60.0,
      },
      ZONA_DE_SEGURIDAD: {
        sku: 'NOM026-EMG-ZONE',
        name: 'Señal Punto de Reunión / Zona de Seguridad',
        unitCost: 280.0,
        installCost: 85.0,
      },
      PRIMEROS_AUXILIOS: {
        sku: 'NOM026-EMG-AID',
        name: 'Señal Estación Primeros Auxilios',
        unitCost: 150.0,
        installCost: 55.0,
      },
    };

    const counts: Record<NOM026SignCategory, { count: number; materials: string[]; mountings: string[] }> = {
      SALIDA_DE_EMERGENCIA: { count: 0, materials: [], mountings: [] },
      RUTA_DE_EVACUACION: { count: 0, materials: [], mountings: [] },
      ESCALERA_DE_EMERGENCIA: { count: 0, materials: [], mountings: [] },
      ZONA_DE_SEGURIDAD: { count: 0, materials: [], mountings: [] },
      PRIMEROS_AUXILIOS: { count: 0, materials: [], mountings: [] },
    };

    store.devices.forEach((d) => {
      if (counts[d.category]) {
        counts[d.category].count += 1;
        if (d.material && !counts[d.category].materials.includes(d.material)) {
          counts[d.category].materials.push(d.material);
        }
        if (d.mountingType && !counts[d.category].mountings.includes(d.mountingType)) {
          counts[d.category].mountings.push(d.mountingType);
        }
      }
    });

    return (Object.keys(counts) as NOM026SignCategory[])
      .filter((cat) => counts[cat].count > 0)
      .map((cat) => {
        const info = categoryDetails[cat];
        const count = counts[cat].count;
        const mat = counts[cat].materials[0] || 'ACRILICO_FOTOLUMINISCENTE';
        const mnt = counts[cat].mountings[0] || 'SOBRE_PUERTA';
        return {
          sku: info.sku,
          name: info.name,
          category: cat,
          ...
          installCost: info.installCost,
          totalAmount: count * (info.unitCost + info.installCost),
          mountingType: mnt,
        };
      });
  }, [store.devices]);

  const grandTotalCost = itemsWithPrice.reduce((sum, item) => sum + item.totalAmount, 0);
  ...
  return (
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-3xl bg-stone-900 text-stone-100 border-stone-800 p-6 rounded-2xl shadow-2xl max-h-[85vh] overflow-y-auto custom-scrollbar">
          ...
          <div className="pt-4 border-t border-stone-800 flex items-center justify-between">
            <Button
              size="sm"
              onClick={() => setShowParametricReport(true)}
              className="h-9 text-xs bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold gap-1.5 shadow-md shadow-emerald-950/50 px-4"
            >
              <FileText className="w-4 h-4 text-emerald-100" /> Generar Reporte Paramétrico
            </Button>

            <Button
              size="sm"
              onClick={() => onOpenChange(false)}
              className="h-9 text-xs bg-stone-800 hover:bg-stone-700 text-stone-200 font-bold px-4 border border-stone-700"
            >
              Cerrar Resumen
            </Button>
          </div>
      ...
      {/* Diálogo del Reporte Paramétrico de Señalización NOM-026 */}
      <EmergencyExitParametricReportDialog
        open={showParametricReport}
        onOpenChange={setShowParametricReport}
