...
interface EmailEmergencyExitDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  itemsWithPrice: EmergencySignagePdfItem[];
  meta: EmergencySignagePdfMetadata;
}

function buildEmailBody(meta: EmergencySignagePdfMetadata, items: EmergencySignagePdfItem[]): string {
  ...
  const breakdownLines = items
    .map(
      (it, idx) =>
        `  ${idx + 1}. [${it.sku}] ${it.name} (${it.material}) | Cant: ${it.count} PZA | Mat: ${formatCurrency(it.unitCost, currency)} | Inst: ${formatCurrency(it.installCost, currency)} | Importe: ${formatCurrency(it.totalAmount, currency)}`
    )
    .join('\n');

  return `Estimado/a cliente / equipo de trabajo,

Por medio del presente correo, me permito compartirles el Dictamen de Estimación Paramétrica del Sistema de Señalización de Emergencia y Rutas de Evacuación (NOM-026-STPS-2008) del proyecto indicado a continuación.
...
--------------------------------------------------
  RESUMEN FINANCIERO Y CUMPLIMIENTO NORMAS (NOM-026-STPS-2008)
--------------------------------------------------
  * Total Señales       : ${meta.totalCount} señales en plano 2D
  * Validaciones NOM    : ${meta.validCount} de ${meta.totalCount} validadas (${meta.compliancePercent}% Cumplimiento)
  * Subtotal Materiales : ${formatCurrency(meta.subtotalMaterials, currency)}
  * Subtotal Instalación: ${formatCurrency(meta.subtotalInstall, currency)}
  * Subtotal General    : ${formatCurrency(meta.subtotalAmount, currency)}
...
--------------------------------------------------
  DESGLOSE DE SEÑALIZACIÓN Y CUANTIFICACIÓN
--------------------------------------------------
...
export function EmailEmergencyExitDialog({
  open,
  onOpenChange,
  itemsWithPrice,
  meta,
}: EmailEmergencyExitDialogProps) {
  const [to, setTo] = useState('');
  const [ccList, setCcList] = useState<string[]>([]);
  const [ccInput, setCcInput] = useState('');
  const [subject, setSubject] = useState(
    `Dictamen Paramétrico Señalización NOM-026${meta.projectName ? ` | ${meta.projectName}` : ''}`
  );
  ...
  const cleanProjectName = (meta.projectName || 'Proyecto')
    ...
    .replace(/_+/g, '_');
  const attachmentName = `${cleanProjectName}_Parametrico_Senalizacion_NOM026.pdf`;
  ...
  const handleDownloadPdf = () => {
    ...
    try {
      exportEmergencySignageReportToPDF(itemsWithPrice, meta);
    ...
    <DialogPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DialogPrimitive.Portal>
        ...
        >
          ...
          <div className="shrink-0 bg-gradient-to-r from-[#0a2e1a] via-[#133d25] to-[#0a2e1a] px-5 py-4 sm:px-8 sm:py-5 border-b border-emerald-500/30">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-1.5 text-xs text-emerald-200/80 font-mono">
              ...
              <span className="flex items-center gap-1.5">
                ...
                {meta.validCount} de {meta.totalCount} señales validadas ({meta.compliancePercent}%)
              </span>
            </div>
          </div>
            ...
            <div className="lg:w-[420px] xl:w-[460px] shrink-0 flex flex-col border-r border-stone-800 overflow-y-auto">
              <div className="p-4 sm:p-6 space-y-4 sm:space-y-5 flex-1">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    id="email-to-emergency"
                    type="email"
                    value={to}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setTo(e.target.value)}
                    placeholder="destinatario@cliente.com"
                    className="h-11 text-sm bg-stone-900 border-stone-700 text-white focus-visible:ring-emerald-500"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      onClick={addCc}
                      className="h-11 w-11 p-0 border-stone-700 text-stone-300 hover:text-emerald-400 hover:border-emerald-500 bg-stone-900"
                    >
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3 p-4 bg-stone-900 border border-stone-800 rounded-xl">
                    <div className="w-11 h-11 rounded-xl bg-emerald-950/80 border border-emerald-500/40 flex items-center justify-center shrink-0">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] text-stone-400 mt-0.5">Reporte Oficial GESTION DE RIESGOS Y NORMAS STPS</p>
                    </div>
                    <Button
                      type="button"
                      variant="outline"
                      ...
                      className="h-9 px-3 text-xs text-emerald-300 border-emerald-500/50 bg-stone-900 hover:bg-stone-800 shrink-0 font-semibold"
                    >
            ...
            <div className="flex-1 min-w-0 flex flex-col overflow-hidden bg-stone-900/60 p-6 space-y-4">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-stone-300 uppercase tracking-wider flex items-center gap-2">
                  <Mail className="w-4 h-4 text-emerald-400" /> Vista Previa del Correo (Outlook Mockup)
                </span>
                <Badge variant="outline" className="text-[10px] text-emerald-300 border-emerald-500/40 font-mono">
                  Outlook Draft Mode
                </Badge>
              </div>

              {/* Contenedor Mockup Outlook */}
              <div className="flex-1 overflow-hidden flex flex-col border border-stone-800 rounded-xl shadow-2xl bg-stone-950">
                {/* Barra Azul Outlook */}
                ...
                <div className="flex-1 overflow-y-auto p-4 bg-stone-900/90 text-stone-200 text-xs font-mono leading-relaxed whitespace-pre-wrap">
                  {previewBody}
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-3.5 bg-amber-950/40 border border-amber-500/40 rounded-xl text-xs text-amber-200 leading-relaxed">
                ...
                <p>
                  <strong>Importante:</strong> Al presionar <strong>Enviar con Outlook</strong>, se descargará la estimación paramétrica NOM-026 en PDF y se abrirá tu borrador en Outlook listo para adjuntar.
                </p>
              </div>
            </div>
          ...
          {/* ══ FOOTER ══════════════════════════════════════════════════════ */}
          <div className="shrink-0 border-t border-stone-800 bg-stone-950 px-8 py-4 flex items-center justify-between gap-4">
            <p className="text-xs text-stone-400">
              Low Voltage Estimator · Módulo Paramétrico Señalización & Evacuación (NOM-026-STPS-2008)
            </p>
